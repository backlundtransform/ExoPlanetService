...
namespace ExoplanetService.Managers
{
    ...
    {
        private  string exoplaneturl = ConfigurationManager.AppSettings["Exourl"];

        private List<Constellation> _constellations = ConfigurationManager.AppSettings["Constellations"].Split(new char[] { ';' }).Select(p => new Constellation { Name = p }).ToList();
