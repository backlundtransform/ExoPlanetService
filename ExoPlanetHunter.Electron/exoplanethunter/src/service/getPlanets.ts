...
export const GetStatisticsAsync = async (): Promise<statistics> => {
  const stat = await fetch(`${url}/api/Statistics`)
    .then(response => {
...
export const GetPlanetListAsync = (filter: filter, top: number) => {
  ...
  const planetList = fetch(
    `${url}/api/ExoSolarSystems/GetPaginatedPlanets?page=${page -
...
export const GetPlanetAsync = (name: string) => {
  const planetList = fetch(
    `${url}/api/ExoSolarSystems/ExoPlanets?%24filter=Name eq '${encodeURIComponent(
      name
    )}'`
  )
    .then(response => {
...
export const GetHabitablePlanets = async () => {
  const planetList = await fetch(`${url}/api/ExoSolarSystems/GetHabitablePlanets`)
    .then(response => {
      return response.json()
    })
    .then(myJson => {
