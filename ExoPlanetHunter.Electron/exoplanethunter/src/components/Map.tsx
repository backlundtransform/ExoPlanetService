...
export default class Map extends React.Component<any, StarMapState> {
    ...
    planets.map(planet =>  {
      ...
      planetmarker.addEventListener('click', () =>
        this.props.history.push({
          pathname: `system/${planet.star&&planet.star.name}`,
    ...
    this._map.on('zoomend', (e: any) => {
      ...
      if (zoom === maxzoom) {
        ...
        if (nearestplanet.distance < 0.5) {
          this.props.history.push({
            pathname: `system/${nearestplanet.star&&nearestplanet.star.name}`,
  ...
  updatemarker = () => {
    ...
    this._markers.map(marker =>this._map&& this._map.removeLayer(marker))
