...
export const getSolarSystem=async (star:Star):Promise<Star>=>{
  ...
  const SolarSystems =   await getData(`../api/ExoSolarSystems/GetExoSolarSystemByName?name=${encodeURIComponent(star.name)}`)
...
export const ConstellationSolarSystems=async (constellation:number):Promise<Array<Star>>=>{
  ...
  const SolarSystems =   await getData(`../api/ExoSolarSystems/GetSolarSystemPerConstellation?constellation=${constellation}`)
