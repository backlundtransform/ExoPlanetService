...
const planetList =   fetch(`../api/ExoSolarSystems/ExoPlanets?${filterstring}&%24top=${top}&%24skip=${skip}&%24orderby=${orderby}`)
  .then((response) => {
    return response.json();
  })
 ...
 export const GetPlanetAsync=   (name:string) => {
  ...
  const planetList =  fetch(`../api/ExoSolarSystems/ExoPlanets?%24filter=Name eq '${encodeURIComponent(name)}'`)
  .then((response) => {
    return response.json();
  })
  .then((myJson):Planet =>  {
